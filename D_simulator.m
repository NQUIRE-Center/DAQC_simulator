...
%DA_SIMULATOR Function to simulate the application of a digital circuit in a noisy platform.
...
%   (rx/rz/CNOT)gatetime  	Time for the application of the corresponding SQG. In units of s.
...
%
%   OUTPUT:
%    rho                    Final density matrix before measurement
...
% We can precalculate the bit-flip channel renormalization matrix, exactly for the SQGs, approximately for the CNOT gate
...
pbf_CNOT = (1-exp(-2*ceil(CNOTgatetime/rzgatetime)*pbf))/2;

% Check if there is any checkpointFile
% % % if exist('chkD.mat','file')==2
% % %     load('chkD.mat')
% % %     str = chk;
% % % else
    str = 1;
...
% % %     tic%kk
    %-----------------------------------------------------------------------------------------------------------------------
    % UNITARY EVOLUTIONS AND ERRORS
    %-----------------------------------------------------------------------------------------------------------------------
    if gate_type(i)=="rz" || gate_type(i)=="rx"
        % Calculate the digital block time
        ...
        U = USQG(angles(:,i),rD,deltaT,gate_type(i));% expm(-1i*gen_HSQG(angles(:,i),rD,deltaT,gate_type(i)));
    elseif gate_type(i)=="CNOT"
        % Calculate the digital block time
        deltaT = CNOTgatetime;
        % Calculate the unitary evolution with the corresponding unitary errors
        U = UCNOT(CNOT_ctrl(:,i),CNOT_targ(:,i),CNOTerror);
    else
	    error("Invalid gate: "+gate_type(i))
    end
    if gate_type(i)=="rz"
    ...
    Uidealaux = Uideal_step(gate_type(i),angles(:,i),CNOT_ctrl(:,i),CNOT_targ(:,i));
    % We check if we can write this matrix as sparse to multiply it faster
    if gate_type(i)=="rz"
        % If the unitary matrix is diagonal, we wan make the operation much faster
        Uideal = transpose(Uidealaux).*Uideal;
    else
    ...
    try%#ok<TRYNC>
        % Update the density matrix if we have applied a quantum channel
        rho = rhoaux;
        clear rhoaux
    end
...
% It is more efficient to trace out the qubits one by one
auxN = N;
rhomeas = rho;
...
%---------------------------------------------------------------------------------------------------------------------------
function res = UCNOT(CNOT_ctrl,CNOT_targ,CNOTerror)
% Auxiliary function to obtain the noisy Hamiltonian corresponding to the CNOT gates
N = length(CNOT_ctrl);
if CNOTerror == 0
    % If we have no error we can just call the ideal function, which is much faster
    res = Uideal_step("CNOT",zeros(N,1),CNOT_ctrl,CNOT_targ);
else
    res = sparse(2^N,2^N);
    for j = nonzeros(CNOT_ctrl(:))'
        ctrl = find(CNOT_ctrl(:)==j);
        targ = find(CNOT_targ(:)==j);
        % We simulate each of the CNOT gates with an individual noise
        % To do less calculations, we do the trick of substracting the identity to the full CNOT hamiltonian. This only changes the result by a global phase
        if ctrl < targ
            res = res + normrnd(pi/2,CNOTerror)*(kron(speye(2^(ctrl-1)),kron(sparse([0 0;0  1]),kron(speye(2^(targ-ctrl-1)),kron([-1 1;1 -1],speye(2^(N-targ)))))));
        else
            res = res + normrnd(pi/2,CNOTerror)*(kron(speye(2^(targ-1)),kron([-1 1;1 -1],kron(speye(2^(ctrl-targ-1)),kron(sparse([0 0;0  1]),speye(2^(N-ctrl)))))));
        end
    end
    res = expm(-1i*full(res));
end
end
%---------------------------------------------------------------------------------------------------------------------------
function res = Uideal_step(gate_type,angles,CNOT_ctrl,CNOT_targ)
% Auxiliary function to generate the unitary ideal evolution of the circuit
if gate_type=="rx" || gate_type=="rz"
	res = USQG(angles,0,0,gate_type);
elseif gate_type=="CNOT"
    ...
    for j = nonzeros(CNOT_ctrl(:))'
        ...
        targ = find(CNOT_targ(:)==j);
        if ctrl < targ
            res = res * (speye(2^N) + kron(speye(2^(ctrl-1)),kron(sparse([ 0 0;0  1]),kron(speye(2^(targ-ctrl-1)),kron([-1 1;1 -1],speye(2^(N-targ)))))));
        else
            res = res * (speye(2^N) + kron(speye(2^(targ-1)),kron([-1 1;1 -1],kron(speye(2^(ctrl-targ-1)),kron(sparse([ 0 0;0  1]),speye(2^(N-ctrl)))))));
        end
    end
end
end

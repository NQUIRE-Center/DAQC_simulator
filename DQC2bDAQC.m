...
clear Mmat b t_alpha tmin angles_2QG i j k t_alpha_validIndex n m pairs_list
...
angles_SQG = mod(NEWangles_SQG,2*pi);
analog_time = NEWanalog_time;
clear auxblock NEWblock_type NEWangles_SQG NEWanalog_time i auxi
            
            
%---------------------------------------------------------------------------------------------------------------------------
% WRITE EVERYTHING AS A BANGED DIGITAL ANALOG BLOCKS SCHEDULE
%---------------------------------------------------------------------------------------------------------------------------
...
time = [];
angles = [];
analog_block_time = 0; %The time stamps for finishing the current analog block
digital_block_time = 0; %The earliest time at which we can apply a digital block
...
i = 1;
len_block_type = length(block_type); % Calculate this before as we will be using it repeatedly
while  i <= len_block_type
    if block_type(i) == "zz"
        analog_block_time = analog_block_time + round(analog_time(i)/minstep)*minstep;
        i = i+1;
    elseif block_type(i) == "rx" || block_type(i) == "rz"
        % First we obtain the total SQG block time
        SQG_blocktime = 0;
        j = 0;
        while i+j<=len_block_type && block_type(i+j) ~= "zz"
            SQG_blocktime = SQG_blocktime + (block_type(i+j)=="rx")*rxgatetime + (block_type(i+j)=="rz")*rzgatetime;
            j = j+1;
        end
        % Check if there is an overlap between analog blocks
        if digital_block_time > analog_block_time - round(SQG_blocktime/(2*minstep))*minstep && ~isempty(gate_type)
            warning("timeCutThres too short to accomodate the digital blocks.")
            warning("off")
        end
        digital_block_time = analog_block_time - round(SQG_blocktime/(2*minstep))*minstep;
        % We now write all the SQGs in order
        while i<=len_block_type && block_type(i) ~= "zz" 
            if isempty(gate_type)
                % Corretly put the initial time at 0 for the first digital block
                digital_block_time = 0;
            end
            gate_type(end+1) = block_type(i);
            time(end+1) = digital_block_time;
            digital_block_time = digital_block_time + (block_type(i)=="rx")*round(rxgatetime/minstep)*minstep + (block_type(i)=="rz")*round(rzgatetime/minstep)*minstep;
            angles(:,end+1) = angles_SQG(:,i);
            i = i+1;
        end
    end
end
% We add a final block marking the end of the schedule (this is usefull for marking the total time of the circuit)
gate_type(end+1) = "END";
time(end+1) = time(end) + analog_time(end)*(block_type(end-1)=="zz")+rxgatetime*(block_type(end-1)=="rx")+rzgatetime*(block_type(end-1)=="rz");
angles(:,end+1) = zeros(N,1);
warning("on")
clear i j analog_block_time SQG_blocktime digital_block_time
end
